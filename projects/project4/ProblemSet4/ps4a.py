...
def getWordScore(word, n):
    ...
    if word == '' or word == '.':
        ...
        if len(word) == n:
            for elem in word:
                score += SCRABBLE_LETTER_VALUES[elem]
            score *= n
            score += 50
            return score
        else:
...
def updateHand(hand, word):
    ...
    copyhand = hand.copy()
    for elem in word:
        copyhand[elem] = copyhand.get(elem)-1
    #print('copehand',copyhand)
    return copyhand
...
def isValidWord(word, hand, wordList):
    ...
    if word in wordList:
        ...
        for k in temp.keys():
            if not k in hand.keys() or temp[k] > hand[k]:
                return False
        return True
    else:
        return False
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(hand)>0:
        newhand = ''
        for key in hand:
        #for key, count in hand.iteritems():
            newhand +=key*hand[key]
        print('Current Hand:'," ".join(newhand))
        ...
        # Otherwise (the input is not a single period):
            ...
            if not isValidWord(userinput, hand, wordList):
                ...
                total += score
                print(userinput,'earned', score,'points.','Total:',total,'points')
    ...
    while True:
        ...
        if userInput == 'r':
            if not PreHand:
                ...
                playHand(PreHand, wordList, HAND_SIZE)
...
if __name__ == '__main__':
    ...
    #playHand({'d': 1, 'i': 1, 'k': 1, 'q': 1, 's': 1, 't': 1, 'u': 1, 'v': 1, 'z': 2}, wordList, 10)
    playGame(wordList)
